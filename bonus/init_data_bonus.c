...
/*   Updated: 2025/02/02 16:18:26 by obouizi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_data(t_data *mlx)
{
	...
	mlx->moves_str = NULL;
	mlx->buffer_img = NULL;
	ft_memset(mlx->player->sprites_left, 0,
		sizeof(mlx->player->sprites_left));
	ft_memset(mlx->player->sprites_right, 0,
		sizeof(mlx->player->sprites_right));
	ft_memset(mlx->digits, 0, sizeof(mlx->digits));
	mlx->collect_all = 0;
	mlx->frame = 0;
	mlx->enemy_count = 0;
...
void	init_buffer_img(t_data *mlx)
{
	...
	if (!mlx->buffer_img)
	{
		perror("Error\nAllocation failed for buffer_img structure! :");
		close_window(mlx);
	}
